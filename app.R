...
mvdataC=read.csv("Sector_C.csv", header=TRUE, sep=",")
mvdataA=read.csv("Sector_A.csv", header=TRUE, sep=",")
mvdataRentA =read.csv("Sector_rent.csv", header=TRUE, sep=",")
...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    menuItem('Sell',
             ...
             menuSubItem('Sector A',
                         tabName = 'SectorA',
                         icon = icon('line-chart')),
             
             menuSubItem('Sector C',
                         tabName = 'SectorC',
                         icon = icon('line-chart')
...
frow2 <- fluidRow(align = "center",
  br(),
  bootstrapPage(
                column( 8,titlePanel(HTML("<h><center><font size=5> Bahria Sector Plot Map </font></center></h>")),
                        leafletOutput(outputId = 'MapPlot2'), br(),  br()),
                column( 4,titlePanel(HTML("<h><center><font size=5> Bahria Sector Street Map </font></center></h>")),
                        m
                        )
  ...
  titlePanel(HTML("<h><center><font size=5> General Records </font></center></h>")),
  br()
)
fr <- fluidRow (
  br(),
...
frRentA <- fluidRow (
  ...
  br(),
  titlePanel(HTML("<h><center><font size=5> General Records </font></center></h>")),
  dataTableOutput('dataTableRentA'),
  br()
)
...
body <- dashboardBody(
  tabItems(
    ...
    tabItem("SectorC",frow1, frC),
    tabItem("SectorA",frow1, frA),
    tabItem("RentSectorA",frow1, frRentA)
  )
)
...
server <- function(input, output) {
  ...
  output$dataTableC <- renderDT(
    datatable(mvdataC, options=list(columnDefs = list(list(visible=FALSE, targets=c(3,7,8,9,10,11,13,15,18,21,22,23,24) )),scrollX = TRUE)), # data
    class = "display nowrap compact", # style
    filter = "top" # location of column filters
    
  )
  
  output$dataTableA <- renderDT(
    datatable(mvdataA, options=list(columnDefs = list(list(visible=FALSE, targets=c(3,7,8,9,10,11,13,15,18,21,22,23,24) )),scrollX = TRUE)), # data
    class = "display nowrap compact", # style
    filter = "top" # location of column filters
    
  )
  output$dataTableRentA <- renderDT(
    datatable(mvdataRentA, options=list(columnDefs = list(list(visible=FALSE, targets=c(3,7,8,9,10,11,13,15,18,21,22,23,24) )),scrollX = TRUE)), # data
    class = "display nowrap compact", # style
    filter = "top" # location of column filters
  ...
  div(style="display:inline-block",
      ...
      observeEvent (input$Southopen,{
        ...
        leafletProxy("MapPlot2")%>% clearMarkers() %>%
          addCircleMarkers(data = filteredm,radius = 10,color = ~pal1(MEDV),fill = TRUE, fillColor = ~pal1(MEDV),fillOpacity = 0.8,popup = leafpop::popupTable(filteredm,zcol = cbind("Name","PLT","SOUTH","HGW","PARK","SCH","PLAZA","HOSP","MASJID","NALLAH","FILTER","MEDV"),feature.id = FALSE,row.numbers = FALSE))
      }))
